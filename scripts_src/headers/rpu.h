...
default : return false;
        ...
        procedure
        is_lockpick_mech(variable item) begin
    switch (obj_pid(item)) begin
    case PID_LOCKPICKS:
return true;
...
default : return false;
        ...
        procedure
        is_lockpick(variable item) begin
    if is_lockpick_mech (item) then return true;
if is_lockpick_elec (item)
  then return true;
return false;
end
         ...
         */
        procedure
        dude_is_addicted() begin
    if global_var (GVAR_ADDICT_JET) or
    global_var(GVAR_NUKA_COLA_ADDICT) or global_var(GVAR_BUFF_OUT_ADDICT) or global_var(GVAR_MENTATS_ADDICT) or global_var(GVAR_PSYCHO_ADDICT) or global_var(GVAR_RADAWAY_ADDICT) or global_var(GVAR_ALCOHOL_ADDICT) or global_var(GVAR_ADDICT_TRAGIC) then return true;
return false;
end
